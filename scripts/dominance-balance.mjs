...
function finalizeAggregate(aggregate, initialOrganisms) {
  ...
  const score =
    avgLineageEntropy * 0.14 +
    tailLineageEntropy * 0.13 +
    avgSpeciesEntropy * 0.08 +
    tailSpeciesEntropy * 0.06 +
    (avgLineages / initialOrganisms) * 0.03 +
    (tailLineages / initialOrganisms) * 0.03 +
    lineagePersistence * 0.08 +
    speciesPersistence * 0.05 +
    saturate(avgPersistentLineages, 18) * 0.04 +
    saturate(avgEventRate, 18) * 0.06 +
    saturate(avgSpeciationRate, 0.6) * 0.05 +
    saturate(tailSpeciationRate, 0.42) * 0.03 +
    saturate(avgCompositionDelta, 0.26) * 0.09 +
    saturate(dominantSwitchRate, 0.16) * 0.12 +
    saturate(avgSpeed, 11) * 0.04 +
    saturate(tailSpeed, 11) * 0.04 +
    (1 - saturate(avgDominantShare, 0.32)) * 0.04 +
    (1 - saturate(tailDominantShare, 0.28)) * 0.07 -
    collapseRate * 0.18;
  ...
  },
  {
    ...
    override: {
      flocking: {
        ...
        alignmentWeight: 0.9,
        cohesionWeight: 1.18,
        separationWeight: 0.94,
        countInfluence: 0.08
      },
      economy: {
        corpseLootFraction: 0.19,
        globalRespawnFraction: 0.24,
        resourceLifetime: 16
      },
      terrain: {
      ...
      combat: {
        ...
        rangedDamageScale: 0.77,
        shieldBlockFraction: 0.87,
        lootEfficiency: 0.87
      },
      mutation: {
        slotCountChance: 0.04,
        slotTypeChance: 0.072,
        allocationJitter: 0.26,
        traitJitter: 0.21,
        speciationChance: 0.01,
...
printStage("Stage 1 - dominance turnover", stage1);
